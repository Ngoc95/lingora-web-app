...
export default function EditStudySetPage() {
  ...
  const { studySet, isLoading: isLoadingDetail } = useStudySetDetail(id);
  const { update, isLoading: isMutating, error } = useStudySetMutation();
  ...
  // Initialize form from studySet (using ref to avoid lint warning)
  useEffect(() => {
    if (studySet && !initializedRef.current) {
      initializedRef.current = true;
      
      // Update form state in next tick to avoid React warning
      requestAnimationFrame(() => {
        setTitle(studySet.title);
        setDescription(studySet.description || "");
        setVisibility(studySet.visibility);
        setPrice(studySet.price);

        setFlashcards(
          studySet.flashcards.map((fc: Flashcard) => ({
        ...
        setQuizzes(
          studySet.quizzes.map((q: Quiz) => ({
            type: q.type as QuizType,
            question: q.question,
            options: q.options,
            selectedCorrectOptions: parseCorrectAnswers(q.correctAnswer),
          }))
        );
      });
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result) {
      // Use replace so back button goes to list, not edit form
  ...
  if (isLoadingDetail) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-[var(--neutral-50)]">
  ...
  if (!studySet) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4 bg-[var(--neutral-50)]">
        <p className="text-[var(--neutral-600)]">Không tìm thấy học liệu</p>
        <button

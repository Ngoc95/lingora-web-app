      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <Tabs defaultValue="learn" className="w-full">
          <TabsList className="grid w-full grid-cols-2 bg-neutral-100 p-1 rounded-xl">
            <TabsTrigger 
              value="learn" 
              className="flex items-center gap-2 rounded-lg data-[state=active]:bg-white data-[state=active]:text-primary-500 data-[state=active]:shadow-sm transition-all"
            >
            ...
            {unlearned === 0 ? (
              ...
              <>
                ...
                <button
                  ...
                  className="w-full py-4 rounded-xl font-semibold text-lg transition-all duration-200 bg-primary-500 text-white hover:bg-primary-600 shadow-lg shadow-primary-500/20 disabled:bg-neutral-200 disabled:text-neutral-400 disabled:cursor-not-allowed disabled:shadow-none"
                >
                  Bắt đầu học
                </button>
              </>
          ...
          <TabsContent value="list" className="mt-6 space-y-4">
            ...
            {isLoading ? (
              ...
              <div className="text-center py-12">
                <p className="text-neutral-600">Không có từ nào</p>
              </div>
            ) : (
              <>
                <div className="space-y-3">
                  {words.map((w) => (
                    <div
                      key={w.id}
                      className="p-4 rounded-xl bg-white border border-neutral-100 shadow-sm hover:shadow-md transition-shadow"
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {w.progress ? (
                          <span className="flex-shrink-0 text-xs px-2 py-1 rounded-full bg-green-100 text-green-700 font-medium">
                            Đã học
                          </span>
                        ) : (
                          <span className="flex-shrink-0 text-xs px-2 py-1 rounded-full bg-neutral-100 text-neutral-600">
                            Chưa học
                          </span>
                        )}
                      </div>
